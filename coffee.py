...
def main():
    ...
    if coordinates_personal:
        logging.info(f'Ваши координаты: {coordinates_personal}')
    else:
        logging.warning('Координаты не найдены, работа приложения завершена.')
        return
    coffee_shops_info = []
    ...
    if isinstance(coffee_list, list):
        for coffee in coffee_list:
            ...
            coordinates_coffee = coffee.get('geoData', {}).get('coordinates', [None, None])
            longitude = coordinates_coffee[0] if len(coordinates_coffee) > 0 else 'Координаты отсутствуют'
            latitude = coordinates_coffee[1] if len(coordinates_coffee) > 1 else 'Координаты отсутствуют'
            distance_km = distance.distance(coordinates_personal, (latitude, longitude)).km
        ...
        coffee_shops_info = sorted(coffee_shops_info, key=lambda x: x['distance'])
